...
func TestTransferTx(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		require.Equal(t, account2.ID, transfer.TargetAccountID)
		...
		require.Equal(t, account2.ID, TargetAccount.ID)
		// check account balances
		fmt.Println(">> tx:", SourceAccount.Balance, TargetAccount.Balance)

		dff1 := account1.Balance - SourceAccount.Balance
		dff2 := TargetAccount.Balance - account2.Balance
		require.Equal(t, dff1, dff2)
		fmt.Println(">> tx:", dff1)

		require.True(t, dff1 > 0)
		require.True(t, dff1%amount == 0)
